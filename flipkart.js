...
const { findMatch } = require("./search-match.js")
...
const FlipkartSearchProduct = async (req, res) => {
    try {
            ...
            document.querySelectorAll('._1AtVbE.col-12-12').forEach(product => {
                ...
                if (childrens.length === 1) {
                    ...
                    childrens.forEach((child) => {
                        const title = child.querySelector('.s1Q9rs')?.getAttribute('title');
        ...
        if (products.data.length > 0) {
            const similarityData = findMatch(search_query, products.titles);
            products.data.forEach((product, index) => {
                product.similarity = similarityData[index].similarity;
            });
            return res.json({
                data: products.data,
            });
